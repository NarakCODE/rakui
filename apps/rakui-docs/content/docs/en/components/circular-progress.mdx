---
title: Circular Progress
description: A circular progress indicator that displays completion progress in a ring format with support for indeterminate states.
---

A circular progress indicator that displays completion progress in a ring format with support for indeterminate states.

<ComponentTabs name="circular-progress-demo" />

## Installation

<CodeTabs defaultValue="cli">

<TabsList>
  <TabsTrigger value="cli">Command</TabsTrigger>
  <TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">

<CodeBlockTabs defaultValue="npm" groupId="package-manager">
  <CodeBlockTabsList>
    <CodeBlockTabsTrigger value="npm">npm</CodeBlockTabsTrigger>
    <CodeBlockTabsTrigger value="pnpm">pnpm</CodeBlockTabsTrigger>
    <CodeBlockTabsTrigger value="yarn">yarn</CodeBlockTabsTrigger>
    <CodeBlockTabsTrigger value="bun">bun</CodeBlockTabsTrigger>
  </CodeBlockTabsList>

  <CodeBlockTab value="npm">
    ```bash
    npx shadcn@latest add @rakui/circular-progress
    ```
  </CodeBlockTab>

  <CodeBlockTab value="pnpm">
    ```bash
    pnpm dlx shadcn@latest add @rakui/circular-progress
    ```
  </CodeBlockTab>

  <CodeBlockTab value="yarn">
    ```bash
    yarn dlx shadcn@latest add @rakui/circular-progress
    ```
  </CodeBlockTab>

  <CodeBlockTab value="bun">
    ```bash
    bun x shadcn@latest add @rakui/circular-progress
    ```
  </CodeBlockTab>
</CodeBlockTabs>

</TabsContent>

<TabsContent value="manual">

<Steps className="mb-0 pt-2">
<Step>Install the following dependencies:</Step>

<CodeBlockTabs defaultValue="npm" groupId="package-manager">
  <CodeBlockTabsList>
    <CodeBlockTabsTrigger value="npm">npm</CodeBlockTabsTrigger>
    <CodeBlockTabsTrigger value="pnpm">pnpm</CodeBlockTabsTrigger>
    <CodeBlockTabsTrigger value="yarn">yarn</CodeBlockTabsTrigger>
    <CodeBlockTabsTrigger value="bun">bun</CodeBlockTabsTrigger>
  </CodeBlockTabsList>

  <CodeBlockTab value="npm">
    ```bash
    npm install radix-ui
    ```
  </CodeBlockTab>

  <CodeBlockTab value="pnpm">
    ```bash
    pnpm add radix-ui
    ```
  </CodeBlockTab>

  <CodeBlockTab value="yarn">
    ```bash
    yarn add radix-ui
    ```
  </CodeBlockTab>

  <CodeBlockTab value="bun">
    ```bash
    bun add radix-ui
    ```
  </CodeBlockTab>
</CodeBlockTabs>

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource name="circular-progress" />

<Step>Add the following CSS animations to your globals.css file:</Step>

```css title="globals.css"
@theme {
  --animate-spin-around: spin-around 0.8s linear infinite;

  @keyframes spin-around {
    0% {
      transform: rotate(-90deg);
    }
    100% {
      transform: rotate(270deg);
    }
  }
}
```

<Step>Update the import paths to match your project setup.</Step>
</Steps>

</TabsContent>

</CodeTabs>

## Layout

Import the parts and compose them together.

```tsx title="components/ui/circular-progress.tsx"
import {
  CircularProgress,
  CircularProgressIndicator,
  CircularProgressTrack,
  CircularProgressRange,
  CircularProgressValueText,
} from "@/components/ui/circular-progress";

<CircularProgress>
  <CircularProgressIndicator>
    <CircularProgressTrack />
    <CircularProgressRange />
  </CircularProgressIndicator>
  <CircularProgressValueText />
</CircularProgress>
```

Or use the Combined component to get all the parts in one.

```tsx title="components/ui/circular-progress.tsx"
import { CircularProgressCombined } from "@/components/ui/circular-progress";

<CircularProgressCombined />
```

## Usage

### Interactive Demo

A circular progress with interactive controls and simulated upload progress.

<ComponentTabs name="circular-progress-controlled-demo" />

### Colors

Different color themes using Tailwind CSS stroke and text utilities to customize the track, range, and value text colors.

<ComponentTabs name="circular-progress-colors-demo" />

## Theming

The circular progress component uses CSS currentColor for stroke colors, making it easy to theme using Tailwind's text or stroke utilities:

### Track Theming

```tsx title="circular-progress-track-theme.tsx"
<CircularProgressTrack className="text-green-200 dark:text-green-900" />
```

### Range Theming

```tsx title="circular-progress-range-theme.tsx"
<CircularProgressRange className="text-green-500" />
```

### Value Text Theming

```tsx title="circular-progress-text-theme.tsx"
<CircularProgressValueText className="text-green-700 dark:text-green-300" />
```

### Custom Stroke Styles

You can also use Tailwind's stroke utilities directly:

```tsx title="circular-progress-stroke.tsx"
<CircularProgressTrack className="stroke-blue-200" />
<CircularProgressRange className="stroke-blue-500" />
```

## API Reference

### CircularProgress

The main container component for the circular progress.

<TypeTable
  type={{
    value: {
      description: 'Current progress value (null for indeterminate)',
      type: 'number | null | undefined',
    },
    getValueText: {
      description: 'Function to format the value text',
      type: '(value: number, min: number, max: number) => string',
    },
    min: {
      description: 'Minimum progress value',
      type: 'number',
      default: '0',
    },
    max: {
      description: 'Maximum progress value',
      type: 'number',
      default: '100',
    },
    size: {
      description: 'Diameter of the circular progress in pixels',
      type: 'number',
      default: '48',
    },
    thickness: {
      description: 'Thickness of the progress ring in pixels',
      type: 'number',
      default: '4',
    },
    asChild: {
      description: 'Merge props onto child element',
      type: 'boolean',
      default: 'false',
    },
  }}
/>

<DataAttributesTable
  data={[
    {
      title: "[data-state]",
      value: '"indeterminate" | "loading" | "complete"',
    },
    {
      title: "[data-value]",
      value: "The current progress value (only present when not indeterminate).",
    },
    {
      title: "[data-max]",
      value: "The maximum progress value.",
    },
    {
      title: "[data-min]",
      value: "The minimum progress value.",
    },
    {
      title: "[data-percentage]",
      value: "The normalized progress value as a decimal between 0 and 1.",
    },
  ]}
/>

### CircularProgressIndicator

The SVG container that holds the circular progress tracks and ranges.

<DataAttributesTable
  data={[
    {
      title: "[data-state]",
      value: '"indeterminate" | "loading" | "complete"',
    },
    {
      title: "[data-value]",
      value: "The current progress value.",
    },
    {
      title: "[data-max]",
      value: "The maximum progress value.",
    },
    {
      title: "[data-min]",
      value: "The minimum progress value.",
    },
    {
      title: "[data-percentage]",
      value: "The normalized progress value as a decimal between 0 and 1.",
    },
  ]}
/>

### CircularProgressTrack

The background circle that represents the full range of possible values.

<DataAttributesTable
  data={[
    {
      title: "[data-state]",
      value: '"indeterminate" | "loading" | "complete"',
    },
  ]}
/>

### CircularProgressRange

The portion of the circle that represents the current progress value.

<DataAttributesTable
  data={[
    {
      title: "[data-state]",
      value: '"indeterminate" | "loading" | "complete"',
    },
    {
      title: "[data-value]",
      value: "The current progress value.",
    },
    {
      title: "[data-max]",
      value: "The maximum progress value.",
    },
    {
      title: "[data-min]",
      value: "The minimum progress value.",
    },
  ]}
/>

### CircularProgressValueText

The text element that displays the current progress value or custom content.

<TypeTable
  type={{
    asChild: {
      description: 'Merge props onto child element',
      type: 'boolean',
      default: 'false',
    },
  }}
/>

<DataAttributesTable
  data={[
    {
      title: "[data-state]",
      value: '"indeterminate" | "loading" | "complete"',
    },
  ]}
/>

### CircularProgressCombined

The combined component that includes all the parts.

<TypeTable
  type={{
    value: {
      description: 'Current progress value (null for indeterminate)',
      type: 'number | null | undefined',
    },
    getValueText: {
      description: 'Function to format the value text',
      type: '(value: number, min: number, max: number) => string',
    },
    min: {
      description: 'Minimum progress value',
      type: 'number',
      default: '0',
    },
    max: {
      description: 'Maximum progress value',
      type: 'number',
      default: '100',
    },
    size: {
      description: 'Diameter of the circular progress in pixels',
      type: 'number',
      default: '48',
    },
    thickness: {
      description: 'Thickness of the progress ring in pixels',
      type: 'number',
      default: '4',
    },
    asChild: {
      description: 'Merge props onto child element',
      type: 'boolean',
      default: 'false',
    },
  }}
/>

## Accessibility

### Screen Reader Support

- Uses the `progressbar` role for proper screen reader identification
- Provides `aria-valuemin`, `aria-valuemax`, `aria-valuenow`, and `aria-valuetext` attributes
- Supports indeterminate state by omitting `aria-valuenow` when value is null

### Keyboard Interactions

The Circular Progress is a visual indicator and does not have interactive keyboard controls.

## Notes

- The component automatically handles indeterminate states when value is null or undefined
- Progress values are automatically clamped to the valid range between min and max
- Invalid max or value props will log console errors and use fallback values
- The indeterminate animation uses CSS custom properties and can be customized via the `--animate-spin-around` variable
- All stroke colors use `currentColor` by default, making them responsive to text color changes
