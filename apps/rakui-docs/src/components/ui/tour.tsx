...
type TourTarget = string | React.RefObject<HTMLElement | null> | HTMLElement;
...
function useFocusTrap(
  ...
  React.useEffect(() => {
    ...
    function getTabbableEdges() {
      ...
      return [candidates[0], candidates[candidates.length - 1]] as const;
    }
...
function useStore<T>(
  ...
  overrideStore?: Store | null,
): T {
  const contextStore = React.useContext(StoreContext);
  const store = overrideStore ?? contextStore;
...
function getTargetElement(target: TourTarget): HTMLElement | null {
...
function scrollToElement(
  ...
  const offsetValues: Required<ScrollOffset> = {
    top: 100,
    bottom: 100,
    left: 0,
    right: 0,
  ...
  const isInViewport =
    rect.top >= offsetValues.top &&
    rect.bottom <= viewportHeight - offsetValues.bottom &&
    rect.left >= offsetValues.left &&
    rect.right <= viewportWidth - offsetValues.right;

  if (isInViewport) return;

  const elementTop = rect.top + window.scrollY;
  const scrollTop = elementTop - offsetValues.top;
...
function Tour(props: TourProps) {
  ...
  const store: Store = React.useMemo(
    () => ({
      ...
      setState: (key, value) => {
        ...
        if (key === "open" && typeof value === "boolean") {
          ...
          if (value) {
            if (
              ...
              propsRef.current.onValueChange?.(0);
            }
          } else if (
            stateRef.current.value < stateRef.current.steps.length - 1
          ) {
        ...
        if (key === "value" && typeof value === "number") {
          const previousStep = stateRef.current.steps[previousValue];
          const nextStep = stateRef.current.steps[value];

          previousStep?.onStepLeave?.();
    ...
    [listenersRef, propsRef, stateRef, stepIdCounterRef, stepIdsMapRef],
  );

  const open = useStore((state) => state.open, store);
  const activeStep = useStore(
    (state) => state.steps[state.value] ?? null,
    store,
  );
  ...
  React.useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (event.defaultPrevented || activeStep?.required) return;
  ...
  }, [activeStep?.required, open, propsRef, store]);
...
function TourStep(props: TourStepProps) {
  ...
  const middleware = React.useMemo(() => {
    ...
    const detectOverflowOptions = {
      padding,
      boundary: boundary.filter((value): value is Element => value !== null),
  ...
  }, [arrow, resolvedAlignOffset, resolvedSideOffset, stepData]);
  ...
  const stepContextValue = React.useMemo<StepContextValue>(
    ...
    [arrowX, arrowY, cannotCenterArrow, placedAlign, placedSide],
  );

  React.useEffect(() => {
    if (!(open && targetElement && isCurrentStep)) return;
    ...
    function onResize() {
      updateMask(store, currentTarget, context.spotlightPadding);
    }
  ...
  }, [context.spotlightPadding, isCurrentStep, open, store, targetElement]);
  ...
  }, [context, isCurrentStep, open, stepData?.required, store]);
  ...
  React.useEffect(() => {
    ...
    function onFocusIn(event: FocusEvent) {
      ...
      const isFocusInStep = stepElement?.contains(target) ?? false;
      const isFocusInTarget = targetElement?.contains(target) ?? false;
  ...
  }, [context, isCurrentStep, open, stepData?.required, store, targetElement]);
  ...
  }, [isCurrentStep, open, targetElement]);
...
function TourSpotlightRing(props: TourSpotlightRingProps) {
  ...
  return (
    <RingPrimitive
      ...
      {...ringProps}
      className={cn(
        "pointer-events-none fixed z-50 rounded-xl border-ring ring-[3px] ring-ring/50",
        className,
      )}
      style={{
...
function TourClose(props: TourCloseProps) {
  ...
  const currentStep = useStore((state) => state.steps[state.value] ?? null);

  const onClick = React.useCallback(
    (event: React.MouseEvent<CloseElement>) => {
      onClickProp?.(event);
      if (event.defaultPrevented || currentStep?.required) return;

      store.setState("open", false);
    },
    [currentStep?.required, onClickProp, store],
  );
  ...
  return (
    <ClosePrimitive
      ...
      className={cn(
        "absolute right-4 top-4 rounded-xs opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-hidden focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-40 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
      onClick={onClick}
      {...closeButtonProps}
    ...
    >
      {children ?? (
        <>
          ...
          {!isLastStep && <ChevronRight data-icon="inline-end" />}
        </>
      )}
    </Button>
  );
...
function TourSkip(props: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      data-slot="tour-skip"
      variant="outline"
      isDisabled={currentStep?.required}
      {...skipButtonProps}
      onClick={onClick}
    >
      {children ?? "Skip"}
