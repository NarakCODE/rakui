...
interface SelectionToolbarProps extends DivProps {
  ...
  container?: HTMLElement | ContainerRef | null;
...
function SelectionToolbar(props: SelectionToolbarProps) {
  ...
  const detectOverflowOptions = React.useMemo(
    ...
    [boundary, collisionPadding, hasExplicitBoundaries],
  );
  ...
  const middleware = React.useMemo<Array<Middleware | false>>(
    () => [
      ...
      avoidCollisions
        ...
        : false,
      avoidCollisions ? flip({ ...detectOverflowOptions }) : false,
      sizeMiddleware,
      transformOrigin,
      hideWhenDetached
  ...
  const closeToolbar = React.useCallback(() => {
    ...
    if (!state.open && !state.selectedText && !state.selectionRect) return;
  ...
  }, [closeToolbar, containerProp, getContainer, store]);
  ...
  }, [closeToolbar, getContainer, scheduleUpdate, updateSelection]);
  ...
  }, [clearSelection, open, refs.floating]);
        ...
        >
          <RootPrimitive
            ...
            className={cn(
              ...
              isPositioned &&
                "animate-in fade-in-0 zoom-in-95 duration-200 [animation-timing-function:cubic-bezier(0.16,1,0.3,1)]",
...
function SelectionToolbarItem(props: SelectionToolbarItemProps) {
  ...
  const pointerTypeRef =
    React.useRef<React.PointerEvent<ItemElement>["pointerType"]>("touch");
